...
The six failures are q01, q02, q17, q18, q25, and q26. Both native ClickHouse and forwarded execution returned HTTP 500 with memory-limit errors under the 4 GiB server limit. The existing counter SQL uses indexed `arraySum` expressions that trigger large intermediate allocations. These failures are not counted as successful fallback or as an ASAP benefit.
