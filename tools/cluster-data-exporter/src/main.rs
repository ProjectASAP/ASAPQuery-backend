...
fn setup_logging(
    ...
    if let Some(dir) = log_dir {
        ...
        let file_appender = tracing_appender::rolling::never(dir, "cluster_data_exporter.log");
...
async fn main() -> Result<(), BoxedErr> {
    let cli = Cli::parse();
    let _ = utilities::EXIT_AFTER_EOF.set(std::time::Duration::from_millis(cli.exit_after_eof_ms));
    ...
    info!("Port: {}", cli.port);
    info!("Exit-after-EOF grace period: {}ms", cli.exit_after_eof_ms);
    ...
    match cli.provider {
        ProviderCmd::Google {
            ...
            info!(
                "Parts mode: {}",
                if all_parts { "all-parts" } else { "part-index" }
            );
