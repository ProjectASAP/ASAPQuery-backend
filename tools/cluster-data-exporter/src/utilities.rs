...
/// Optional grace period after finite input is exhausted. A zero duration
/// preserves the legacy behavior of exiting during the terminal scrape.
pub static EXIT_AFTER_EOF: OnceLock<Duration> = OnceLock::new();
static EOF_REACHED_AT: OnceLock<Instant> = OnceLock::new();

pub fn should_exit_after_eof() -> bool {
    let timeout = EXIT_AFTER_EOF.get().copied().unwrap_or_default();
    timeout.is_zero()
        || EOF_REACHED_AT
...
pub struct Cli {
    ...
    /// Grace period after the finite input is exhausted before exiting.
    /// Zero preserves the legacy immediate-shutdown behavior.
    #[arg(long, default_value_t = 0)]
    pub exit_after_eof_ms: u64,
