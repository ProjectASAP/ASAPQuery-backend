...
This is an evaluation tool. It is deliberately outside the backend Cargo
workspace: it replays offline CSV traces through Prometheus's normal scrape
path, after which Prometheus may Remote Write into ASAPQuery-backend.

Finite inputs retain the historical immediate shutdown behavior by default.
For a final scrape grace period, pass `--exit-after-eof-ms=<milliseconds>`;
the smoke test uses this option so it can inspect the terminal metric values.
...
Run the synthetic provider smoke checks (no trace dataset required):
...
- --exit-after-eof-ms: Optional grace period after finite input is exhausted
  (default: 0, immediate shutdown)
...
- --speedup: Alibaba replay speedup factor (1 = real time)
...
`compose.yaml` is the smallest end-to-end source example: it runs the exporter
and a Prometheus scraper for Google data. Set `CDE_DATA_DIR` to the preprocessed
task-usage directory, then run:

```bash
CDE_DATA_DIR=/path/to/task/resource/usage docker compose up --build
```

Prometheus is then available at `http://localhost:9090`. To send those samples
to ASAPQuery-backend, configure Remote Write through the backend's existing
profile or demo rather than duplicating backend startup here.
