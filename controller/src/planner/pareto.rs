...
use super::cost_model::{benchmark_table_pub, score_with, SketchCosts};
...
impl Default for ObjectiveWeights {
    fn default() -> Self {
        Self { bandwidth: 1.0, cpu: 0.0, memory: 0.0 }
    }
}
...
impl ObjectiveWeights {
    fn normalised(self) -> Self {
        ...
        if total <= 0.0 {
            return Self { bandwidth: 1.0, cpu: 0.0, memory: 0.0 };
        }
        Self {
...
pub fn pareto_frontier(
    ...
    sorted.sort_by(|a, b| {
        weighted_score(a, w).partial_cmp(&weighted_score(b, w))
...
mod tests {
    ...
    fn default_wc() -> WorkloadCharacteristics { WorkloadCharacteristics::default() }
    ...
    fn quantile_frontier_non_empty_and_kll_present() {
        ...
        let f = pareto_frontier(
            &quantile_workload(), &default_wc(),
            ObjectiveWeights::default(), None,
        );
        assert!(!f.is_empty(), "frontier should not be empty for quantile workload");
        let types: Vec<_> = f.iter().map(|p| &p.sketch_type).collect();
        assert!(types.contains(&&SketchType::KLL), "KLL expected in frontier");
    }
    ...
    fn bandwidth_weight_selects_lowest_bw() {
        let f = pareto_frontier(
            &quantile_workload(), &default_wc(),
            ObjectiveWeights { bandwidth: 1.0, cpu: 0.0, memory: 0.0 }, None,
        );
        let best = select_best(&f, ObjectiveWeights { bandwidth: 1.0, cpu: 0.0, memory: 0.0 }).unwrap();
        for p in &f {
            assert!(
    ...
    fn memory_weight_selects_lowest_memory() {
        let f = pareto_frontier(
            &quantile_workload(), &default_wc(),
            ObjectiveWeights { bandwidth: 0.0, cpu: 0.0, memory: 1.0 }, None,
        );
        let best = select_best(&f, ObjectiveWeights { bandwidth: 0.0, cpu: 0.0, memory: 1.0 }).unwrap();
        for p in &f {
            assert!(
    ...
    fn tight_sla_excludes_inaccurate_sketches() {
        ...
        for p in &f {
            assert!(p.estimated_error <= 0.001,
                "{} error {} exceeds 0.001 SLA", p.sketch_type, p.estimated_error);
        }
    }
            ...
            for j in 0..f.len() {
                ...
                let b_dominates_a =
                    ...
                    && (b.bandwidth_bytes_per_sec < a.bandwidth_bytes_per_sec
                        || b.cpu_micros_per_sample < a.cpu_micros_per_sample
                        || b.memory_bytes          < a.memory_bytes);
                assert!(!b_dominates_a, "{} dominates {} but both are in frontier",
