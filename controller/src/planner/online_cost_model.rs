...
use super::cost_model::{benchmark_table_pub, SketchCosts};
...
impl OnlineSketchCosts {
    ...
    pub fn update(&mut self, observed_bw: f64, observed_cpu: f64) {
        self.bw_bytes_per_series_per_sec = EMA_ALPHA * observed_bw
            + (1.0 - EMA_ALPHA) * self.bw_bytes_per_series_per_sec;
        self.cpu_micros_per_sample = EMA_ALPHA * observed_cpu
            + (1.0 - EMA_ALPHA) * self.cpu_micros_per_sample;
        self.observations += 1;
    }
...
mod tests {
    ...
    fn after_min_obs_online_dominates() {
        ...
        assert!(eff.bytes_per_series_per_sec > 100.0,
    ...
    fn single_observation_has_low_weight() {
        ...
        assert!(eff.bytes_per_series_per_sec < 200.0,
            "single spike should not dominate: {}", eff.bytes_per_series_per_sec);
    }
