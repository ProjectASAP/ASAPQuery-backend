...
Given `streaming_config.yaml` (generated by the planner (in `ASAPCollector/controller/`)), asap-summary-ingest:
...
This automation eliminates manual pipeline creation and ensures consistency with the planner (in `ASAPCollector/controller/`) decisions.

## How It Works

### Input: streaming_config.yaml

The the planner (in `ASAPCollector/controller/`) generates this file describing which sketches to build:
