...
`suites/issue-702.yaml` records the nine expressions manually reported in PR #741. It uses the 60-second `aggregations-dense-cadence` fixture. This is not equivalent to that PR's one-second ingestion experiment: input sample cadence changes the number and values of samples seen by range functions. Add a dedicated one-second dataset before using this suite to reproduce that experiment exactly.
