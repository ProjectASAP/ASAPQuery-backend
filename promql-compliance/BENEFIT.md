...
The ClickHouse SQL is generated by `runner/src/sql.rs` from the same ten case names.
...
dataset; query recurrence comes from the shared suite. Source cadence and query lookback retain
100 ms precision through backend admission. No synthetic complete
...
The inherited level-1 test still rejects the grouped temporal Sum plan and the
quantile-ratio exact fallback. These failures must be
