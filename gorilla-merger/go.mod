module github.com/ProjectASAP/asapquery-backend/gorilla-merger
...
// was compiled against (notably vimeo/galaxycache's Codec interface and the
// grpc fork). We must mirror them here for the Thanos packages we import
// (pkg/store -> cacheutil -> cache, pkg/shipper, pkg/component) to build.
replace (
