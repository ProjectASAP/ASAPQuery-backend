...
use crate::drivers::query::fallback::cold_store::{ColdStore, RawSample};

use super::query_planner::{QueryPlan, QueryStatistic};
...
impl ExactExecutor {
    ...
    pub async fn execute_quantile(
        ...
        let (samples, chunks_fetched) = self.collect_buffered_samples(plan).await?;
        if samples.is_empty() {
    ...
    async fn collect_buffered_samples(
        &self,
        plan: &QueryPlan,
    ) -> Result<(Vec<RawSample>, usize), EngineError> {
        let (start_ms, end_ms) = plan.time_range_ms;
        let chunks = self
            .cold_store
            .list_chunks(&plan.metric, start_ms, end_ms)
