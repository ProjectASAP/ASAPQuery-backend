...
pub(super) fn estimate(
    ...
    for (id, demand) in &manifest.components {
        if id.starts_with("source:") {
            ...
            let retention_ms = demand.implementation["population"]["history_retention_ms"]
                .as_u64()
                .unwrap_or(0);
            let versions = if retention_ms == 0 {
                0.0
            } else {
                (retention_ms as f64 / (cadence * 1000.)).ceil() + 1.0
            };
            let retained_bytes = bytes + versions * (bytes + 1024.0);
            let snapshots = if versions == 0.0 {
                0.0
            } else {
                horizon / cadence
            };
            let (cpu, memory) = match phase {
                ...
                "update" => (
                    updates * CPU_PER_ITEM * cardinality.max(2.0).log2()
                        + snapshots * bytes * CPU_PER_BYTE,
                    0.0,
                ),
                "residency" => (0.0, retained_bytes * horizon),
                "retire" => ((cardinality + snapshots) * CPU_PER_ITEM, 0.0),
            ...
            insert(
                ...
                resources(
                    ...
                    0.0,
                    vec![],
                    json!({"phase":phase,"series":cardinality,"updates":updates,"bytes_per_series":SERIES_BYTES,"historical_versions":versions,"retained_bytes":retained_bytes,"snapshots":snapshots}),
                ),
            )?;
        }
    }
...
mod tests {
    ...
    fn physical_state_resources_prefer_applicable_erp() {
        ...
        let plan = super::super::super::compiler::DeploymentPlanCompiler
