...
|---|---|---|---|
| `Frequency` | Standalone `AggIntent::Frequency { accuracy }` | `TopK`'s `RankingMeasure::Frequency` | **Corrected during Phase 1 implementation (#391): these are unrelated concepts that share a name, not two shapes of the same capability.** `AggIntent::Frequency` is control_plane's standalone point-frequency-via-CMS query (`count(*) WHERE key = k`, bound in `bind_cms_count.rs`, ~12 real call sites); `RankingMeasure::Frequency` classifies what a `TopK` ranks by and has no relation to point-frequency queries. **Both are kept** — `Frequency` as control_plane-only (the "no ASAPController equivalent" exception), `RankingMeasure` adopted additively (a real gap control_plane had, not a replacement for `Frequency`). |
