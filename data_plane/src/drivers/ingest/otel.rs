            ...
            for metric in &scope_metrics.metrics {
                ...
                let dps: Vec<ModifiedOtlpSketchDp> = match &metric.data {
                    Some(Data::Ddsketch(d)) => {
                        let cfg = crate::stores::sketch_db::index::SketchConfig::DDSketch {
                    ...
                    Some(Data::Kllsketch(k)) => {
                        let cfg = crate::stores::sketch_db::index::SketchConfig::Kll { k: k.k };
                        k.data_points
                            .iter()
                            .map(|dp| ModifiedOtlpSketchDp {
                                kind: SketchKind::Kll,
                    ...
                    Some(Data::Countsketch(c)) => {
                        let cfg = crate::stores::sketch_db::index::SketchConfig::CountSketch {
                            rows: c.rows,
                            cols: c.cols,
                        };
                        c.data_points
                    ...
                    Some(Data::Countminsketch(c)) => {
                        let cfg = crate::stores::sketch_db::index::SketchConfig::CountMin {
                            rows: c.rows,
                            cols: c.cols,
                        };
                        c.data_points
                    ...
                    Some(Data::Hllsketch(h)) => {
                        let cfg = crate::stores::sketch_db::index::SketchConfig::Hll {
                ...
                for dp in dps {
                    ...
                    // and its key set IS the group-by KEY set.
                    {
                        ...
                        let label_values: BTreeMap<String, String> = dp
                            ...
                            .map(|(k, v)| (k.clone(), v.clone()))
                            .collect();
                        let window: crate::stores::sketch_db::index::epoch_columnar::TimestampRange = (
...
) -> crate::stores::sketch_db::index::SketchKindHandle {
...
fn encoding_to_handle(encoding: i32) -> Option<crate::stores::sketch_db::index::SketchEncoding> {
    use crate::stores::sketch_db::index::SketchEncoding;
    match encoding {
...
struct ModifiedOtlpSketchDp {
    ...
    container_config: crate::stores::sketch_db::index::SketchConfig,
}
