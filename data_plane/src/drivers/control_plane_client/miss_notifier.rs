//! Client for notifying the control plane of query-side
//! capability misses — the query plane side of PR G.
//!
//! When `ASAPQueryEngine` fails to match a query against any stored
//! aggregation (`find_compatible_aggregation` returns `None`), it
//! fires a fire-and-forget notification to the configured control
//! plane with the `QueryRequirements` that failed to match. The
//! control plane can then decide whether to generate a new sketch
//! plan, push it back to the backend via PR E's
//! `POST /api/v1/streaming-config` endpoint, and to the collector
//! side via OpAMP. The query itself is not retried — it falls
//! through to the existing §5.2 fallback (direct Prometheus read,
//! SQL forwarding, etc.) and returns whatever the fallback provides.
//!
//! ## Why fire-and-forget
//!
//! Retrying the query after the control plane generates a plan would
//! require coordination that is out of scope for PR G:
//!   - the control plane's plan generation is not instant
...
/// Transport-agnostic control-plane notification interface.
#[async_trait]
...
struct CapabilityMissPayload {
    /// Constant tag so the control plane can route this payload across
...
/// HTTP-backed `ControlPlaneClient`. POSTs a JSON-encoded
/// `CapabilityMissPayload` to the configured endpoint with a bounded
/// timeout so a slow or unreachable control plane can't stall the query
...
impl HttpControlPlaneClient {
    /// Construct a client pointing at the control plane's plan endpoint.
    /// `endpoint` should be the full URL, e.g.
    /// `http://control-plane.svc.cluster.local:8080/api/v1/plan`.
...
impl ControlPlaneClient for HttpControlPlaneClient {
    async fn notify_capability_miss(&self, requirements: &QueryRequirements) -> Result<(), String> {
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| format!("control-plane POST send error: {e}"))?;
        if !resp.status().is_success() {
            return Err(format!(
                "control plane returned {} for capability-miss notification",
                resp.status()
            ));
        }
        Ok(())
...
pub fn spawn_capability_miss_notify(
    ...
    tokio::spawn(async move {
        if let Err(e) = client.notify_capability_miss(&requirements).await {
            warn!(
                "capability-miss control-plane notification failed: {} \
                 (metric={}, stats={:?})",
...
mod tests {
    ...
    /// Mock client that records calls, used by ASAPQueryEngine unit
    ...
    async fn spawn_helper_is_noop_when_client_is_none() {
        let none: Option<Arc<dyn ControlPlaneClient>> = None;
        let req = test_requirements();
        // Should not panic.
    ...
    async fn spawn_helper_invokes_client_via_tokio_spawn() {
        ...
        let client: Option<Arc<dyn ControlPlaneClient>> =
            Some(mock.clone() as Arc<dyn ControlPlaneClient>);
        let req = test_requirements();
    ...
    async fn http_client_reports_non_success_status() {
        ...
        let client = HttpControlPlaneClient::new(format!("http://{addr}/api/v1/plan"));
        let req = test_requirements();
